...
- **Asset generation** — keyless local routes generate or queue assets: Dreamina CLI handles local image/video runs, procedural output covers debug textures/placeholders, Codex task manifests hand off work to host-agent automation, and Tripo3D converts images to 3D models.
...
- **Runs on commodity hardware** — any machine with the relevant engine toolchain, Python, local Dreamina login state, and Tripo3D credentials for 3D can run the pipeline.
...
`external-smoke` defaults to non-paid dry-runs for Dreamina command construction and Codex task construction, and skips Tripo3D because it has no dry-run path. Use `external-smoke --yes-charge` only when you explicitly want to submit real Dreamina/Tripo3D provider tasks and accept provider credits.
