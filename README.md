...
- **Asset generation** — keyless/local routes generate or queue assets: ComfyUI Partner/API workflows cover image, video, 3D, and LLM smoke through Comfy credits; Dreamina CLI remains available for image/video runs; procedural output covers debug textures/placeholders; Codex task manifests hand off work to host-agent automation; direct Tripo3D remains a 3D fallback.
...
- **Runs on commodity hardware** — any machine with the relevant engine toolchain, Python, and a local ComfyUI server plus `COMFY_API_KEY` can run the unified Partner/API workflow path.
...
- Provider configuration:
  ...
  - `COMFY_API_KEY` — Comfy Account API key for local ComfyUI paid Partner/API nodes
  - Dreamina CLI login state — required when using Dreamina providers
  - `TRIPO3D_API_KEY` — optional fallback for legacy direct Tripo3D 3D commands
...
bin/godogen-ddm asset image --provider codex --prompt "wet cobblestone" -o assets/img/cobblestone.png
...
bin/godogen-ddm asset image --provider comfy-local --workflow ref-image --prompt "red cube" -o assets/img/ref.png
bin/godogen-ddm asset video --provider comfy-local --workflow image-to-video --image assets/img/ref.png --prompt "turntable" --duration 5 --resolution 480p -o assets/video/ref.mp4
bin/godogen-ddm asset model3d --provider comfy-local --workflow tripo-image-to-3d --image assets/img/ref.png -o assets/models/ref.glb
...
`external-smoke` defaults to non-paid dry-runs for Dreamina command construction, Codex task construction, and Comfy local LLM request construction. Use explicit `asset ... --provider comfy-local` commands for paid Comfy smoke, and use `external-smoke --yes-charge` only when you explicitly want to submit legacy real Dreamina/Tripo3D direct-provider tasks.
