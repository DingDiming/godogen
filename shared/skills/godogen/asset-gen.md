...
For source-repo operator checks, `bin/godogen-ddm external-smoke` creates a procedural first frame and validates Dreamina/OpenAI command construction without submitting paid work. `bin/godogen-ddm external-smoke --yes-charge` is the explicit opt-in path for real Dreamina/OpenAI/Tripo3D smoke and may consume provider credits/API billing.
